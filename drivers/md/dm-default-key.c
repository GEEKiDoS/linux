...
/**
 * struct default_key_c - private data of a default-key target
 ...
 * @key_type: raw or hardware-wrapped key
 * @key_started: whether blk-crypto accepted the key for use
				    ...
				    unsigned int argc, char **argv)
{
	...
	if (as.argc) {
		ti->error = "Unexpected feature arguments";
		return -EINVAL;
	}
...
static int default_key_ctr(struct dm_target *ti, unsigned int argc, char **argv)
{
	...
	if (!ti->len || dkc->iv_offset > U64_MAX - (ti->len - 1) ||
	    (dkc->iv_offset & ((dkc->sector_size >> SECTOR_SHIFT) - 1))) {
		ti->error = "Invalid IV range or alignment";
		err = -EINVAL;
		goto bad;
	}
	...
	dun_bytes = max_t(unsigned int, 1, DIV_ROUND_UP(fls64(dkc->max_dun), 8));
...
static int default_key_map(struct dm_target *ti, struct bio *bio)
{
	...
	/* Newly attached contexts must pass through blk-crypto fallback. */
